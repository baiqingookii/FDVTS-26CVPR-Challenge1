...
class SliceFlattenClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, labels=None):
        ...
        flat_feats = x.reshape(B, -1)
        flat_feats = self.norm(flat_feats)
        logits = self.head(flat_feats)
...
class NPYSubsetDataset(Dataset):
    ...
    def _load_volume(self, p: str):
        ...
        if vol.ndim != 3:
            raise ValueError(f"Expect 3D volume, got {vol.shape} at {p}")
        return vol
...
def main():
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="/remote-home/share/25-jianfabai/medgemma-finetune/save_model/flat_block_classify/3d_ori-and-1b_dataset_mean_scale0.7-1-lr5e-6/checkpoint-864"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/remote-home/share/25-jianfabai/medgemma-finetune/predict_result/cvpr/flatten_block/3d_ori-and-1b_dataset_cls_lr5e-5_scale0.7-1_24_448_448_12_24_checkpoint-27"
    )

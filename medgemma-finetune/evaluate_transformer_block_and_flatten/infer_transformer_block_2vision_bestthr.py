...
def build_name_to_path(root: str) -> Dict[str, str]:
    return {os.path.splitext(os.path.basename(p))[0]: p for p in list_npys_recursively(root)}
...
def find_best_threshold(val_df: pd.DataFrame, num_thresholds: int = 1001):
...
def main():
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="/remote-home/share/25-jianfabai/medgemma-finetune/save_model/transformer_block_2vision/3d_ori-and-1b_dataset_cls_scale0.7-1-lr1e-6/checkpoint-864"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        # default="/remote-home/share/25-jianfabai/medgemma-finetune/predict_result/transformer_block_2vision_eval/covid1b-eva"
        default="/remote-home/share/25-jianfabai/medgemma-finetune/predict_result/cvpr/transformer_2vision_block/3d_ori-and-1b_dataset_cls_lr5e-5_scale0.7-1_24_448_448_12_24_checkpoint-27"
    )
    ...
    print(f"[model-config] max_slices={args.max_slices}")
    print(f"[model-config] use_cls_token={args.use_cls_token}")
    print(f"[model-config] pool={args.pool}")

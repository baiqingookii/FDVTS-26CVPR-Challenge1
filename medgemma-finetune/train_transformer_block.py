...
class SliceTransformerClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, labels=None):
        ...
        # if self.debug_print_count < self.debug_print_max:
        #     with torch.no_grad():
                
        #         delta = (x - x_before).abs().mean().item()
        #         print("encoder delta mean:", delta)

        #         cls_norm = x[:, 0].norm(dim=-1).mean().item()
        #         tok_norm = x[:, 1:].norm(dim=-1).mean().item()
        #         cls_tok_cos = F.cosine_similarity(
        #             x[:, 0], x[:, 1:].mean(dim=1), dim=-1
        #         ).mean().item()

        #         print("cls_norm:", cls_norm)
        #         print("tok_norm:", tok_norm)
        #         print("cls vs token-mean cosine:", cls_tok_cos)

        # ---- encoder debug (print only first few TRAIN forwards on rank0) ----
        x_before = x.detach().clone()

        x_enc = self.slice_encoder(x)   # (B, T, D), inspect BEFORE final norm
        x = self.norm(x_enc)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="/remote-home/share/25-jianfabai/medgemma-finetune/save_model/transformer_block/3d_ori-and-1b_dataset_cls_lr5e-5_scale0.7-1")
    ...
    model = SliceTransformerClassifier(
        ...
        max_slices=24,          
        n_layers=2,             
        n_heads=8,              # 如果 hidden_dim 太小/太大再调
        dropout=0.1,
        use_cls_token=True,

...
parser.add_argument('--visname', '-vis', default='cvpr-pretrain-c2m1-mix-class', help='visname')
parser.add_argument('--batch_size', '-bs', default=6, type=int, help='batch_size')
...
def main():
    ...
    if args.pretrain:
        # ckpt = torch.load('./checkpoints/eccv/*clf_resnest50_con_mix_iccvtest/28.pkl')
        ...
        # ckpt = torch.load('./checkpoints/cvpr/pseudo-model12-mix-clf/1.pkl')
        # ckpt = torch.load('./checkpoints/cvpr/pseudo-model12/14.pkl')
        # ckpt = torch.load('./checkpoints/cvpr/pseudo-model3-mix/9.pkl')
        # ckpt = torch.load('./checkpoints/cvpr/resnest50-noncon-nomix/3.pkl')
    ...
    #     weight_dir = './checkpoints/eccv/*clf_resnest50_con_mix_iccvtest/28.pkl'
    ...
    #先不用
    # val1(target_model,val_loader,0,test_log, optimizer)
    # train the model
...
def val1(net, val_loader, epoch,test_log, optimizer):
    ...
    # 读取中心信息
    df_covid = pd.read_csv('validation_covid.csv')
    df_noncovid = pd.read_csv('validation_non_covid.csv')
    df_covid['label'] = 1
    df_noncovid['label'] = 0
    df_all = pd.concat([df_covid, df_noncovid])

    # 建立文件名到中心的映射
    id2center = dict(zip(df_all['ct_scan_name'], df_all['data_centre']))
    id2true_label = dict(zip(df_all['ct_scan_name'], df_all['label']))

    preds = torch.cat(pred_list).numpy()
    labels = torch.cat(label_list).numpy()

    # 按中心组织样本
    center_pred = defaultdict(list)
    center_true = defaultdict(list)

    for i in range(len(all_ids)):
        scan_id = all_ids[i]
        center = id2center.get(scan_id)
        # print(scan_id)
        # print(center)
        if center is None:
            print(f"Warning: {scan_id} not found in CSV")
            continue
        center_pred[center].append(preds[i])
        center_true[center].append(labels[i])

    # 计算每个中心的平均 F1
    center_f1_scores = {}
    for center in center_pred:
        y_true = center_true[center]
        y_pred = center_pred[center]

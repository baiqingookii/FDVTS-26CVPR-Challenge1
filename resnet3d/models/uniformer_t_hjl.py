...
class Uniformer(nn.Module):
    ...
    """
    def __init__(self, cfg=None, use_con=False, head='mlp', outdim=128):
        super().__init__()
        ...
        if self.use_con:
            ...
            if head == 'linear':
                self.mlp_head = nn.Linear(embed_dim[-1], outdim)
            elif head == 'mlp':
                self.mlp_head = nn.Sequential(
                    nn.Linear(embed_dim[-1], embed_dim[-1]),
                    nn.ReLU(inplace=True),
                    nn.Linear(embed_dim[-1], outdim)
                )
            else:
        ...
        self.patch_embed1_new = SpeicalPatchEmbed(
    ...
    def get_pretrained_model(self, path=None):
        if path:
            ...
            state_dict_3d = self.state_dict()
            for k in checkpoint.keys():
                if k in checkpoint and k in state_dict_3d and checkpoint[k].shape != state_dict_3d[k].shape:
                    if len(state_dict_3d[k].shape) <= 2:
                        #logger.info(f'Ignore: {k}')
                        continue
                    # logger.info(f'Inflate: {k}, {checkpoint[k].shape} => {state_dict_3d[k].shape}')
                    print('inflate', k, checkpoint[k].shape, '=>', state_dict_3d[k].shape)
                    time_dim = state_dict_3d[k].shape[2]
                    checkpoint[k] = self.inflate_weight(checkpoint[k], time_dim)
...
if __name__ == '__main__':
    # set use con with true or false,
    model = Uniformer(use_con=True)
    print('ready loading model from:', args.pretrained_weights)
    state_dict = model.get_pretrained_model(args.pretrained_weights)
    msg = model.load_state_dict(state_dict, strict=False)
    # reset classifier with 2/4 classses

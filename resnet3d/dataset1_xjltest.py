...
class Lung3D_cvpr_patient_supcon(Data.Dataset):
    def __init__(self, train=False, val=False, inference=False, n_classes = 2, supcon=False, cam=False, box_lung=False, seg_sth=None, iccv_test=False, add_mosmed=False):
    ...
    def __getitem__(self, index):
        ...
        if self.val: 
            ...
            if self.img_size == 224:
                # center crop
                img_array = img_array[:, 16:240, 16:240]

            img_array = convert_tensor_float(img_array)
...
def convert_tensor_float(image):
    # [128, 256, 256]
    img_array = np.expand_dims(image, axis=1)
    img_array = np.repeat(img_array, repeats=3, axis=1)
    img_array = torch.from_numpy(img_array).float() #[0~255]
    img_array = normalize(img_array) #[0~1]
    return img_array
...
def augment2(sample, mask, ifhalfcrop=True,ifrandom_resized_crop=True, ifflip=False, ifrotate=False, ifcontrast=True,ifswap = False,filling_value=0):
    if ifhalfcrop:
        start = np.random.randint(0,sample.shape[0]-64)
        sample = sample[start:start+64]
    if ifrandom_resized_crop:
